...
// Notification System
$stmt = $pdo->prepare("SELECT COUNT(*) FROM documents WHERE department = 'ICT' AND is_read = 0");
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload'])) {
    $title = $_POST['title'];
    $department = $_POST['department'] ?? 'ICT'; // Default to IT if not selected
    ...
    if ($fileSize > $maxSize) {
        ...
        header("Location: ICT.php?upload=success");
        exit();
    } else {
...
$stmt = $pdo->prepare("SELECT * FROM documents WHERE department = 'ICT'");
...
<body>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select id="department" name="department">
                <option value="ICT" selected>ICT Department</option>
    ...
    <!-- 3. View Section -->
    <section>
        <h2>ICT Department Documents</h2>
                ...
                <tr>
                    ...
                    <td>
                        <?php if (isset($_GET['edit']) && $_GET['edit'] == $doc['id']): ?>
                            ...
                            <a href="ICT.php">Cancel</a>
                            </form>
                        <?php else: ?>
                            <a href="ICT.php?edit=<?= $doc['id'] ?>">Edit</a>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php endforeach; ?>
    ...
    <script>
        ...
        document.querySelector('.notification-bell').addEventListener('click', function() {
            fetch('mark_read.php?department=ICT')
                .then(() => {
