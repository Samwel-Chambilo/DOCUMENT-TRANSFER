...
// Notification System
$stmt = $pdo->prepare("SELECT COUNT(*) FROM documents WHERE department = 'COICT' AND is_read = 0");
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload'])) {
    $title = $_POST['title'];
    $department = $_POST['department'] ?? 'COICT'; // Default to IT if not selected
    ...
    if ($fileSize > $maxSize) {
        ...
        header("Location: COICT.php?upload=success");
        exit();
    } else {
...
$stmt = $pdo->prepare("SELECT * FROM documents WHERE department = 'COICT'");
...
<body>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select id="department" name="department">
                <option value="COICT" selected>COICT Department</option>
    ...
    <!-- 3. View Section -->
    <section>
        <h2>COICT Department Documents</h2>
                ...
                <tr>
                    ...
                    <td>
                        <?php if (isset($_GET['edit']) && $_GET['edit'] == $doc['id']): ?>
                            ...
                            <a href="COICT.php">Cancel</a>
                            </form>
                        <?php else: ?>
                            <a href="COICT.php?edit=<?= $doc['id'] ?>">Edit</a>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php endforeach; ?>
